...
Vue.component("button-counter", {
  ...
  template:
    '<div class=count><button v-on:click="count++">+</button>{{count}}<button v-on:click="if(count>0){count--;}">-</button><img src=https://fakeimg.pl/20/></div>',
});
...
let single = [
  ...
  },
  {
    ...
    cal: "300cal",
    text:
      "番茄炒蛋不僅好吃，連配色都很鮮豔，一看就讓人食指大動！紅通通的大番茄含有豐富的茄紅素、維生素C、膳食纖維等營養，是很棒的蔬菜，尤其是它特有的茄紅素，是一種天然色素，不但使番茄呈現討喜的鮮紅色，還具有很強的抗氧化功能，可以降低身體的氧化傷害，保持健康與美麗。 番茄含有豐富的茄紅素。",
  },
  {
    id: "8",
    name: "蠔油青江菜",
...
function ricelightBox() {
  ...
  for (let i = 0; i < rice.length; i++) {
    if (rice[i].id == Number(A)) {
      ...
      document.getElementById("li ghtText").innerText = rice[i].text;
    }
  }
}
document
  .getElementsByClassName("closelightbox1")[0]
  .addEventListener("click", closelightBox1);
...
for (var j = 0; j < meat.length; j++) {
  document
    ...
    .addEventListener("click", meatlightBox);
  document
    .getElementById(`meatplus${j + 1}`)
    .addEventListener("click", meatplusone);
}
// flex-wrap: wrap;
...
document
  .getElementsByClassName("closelightbox0")[0]
  .addEventListener("click", closelightBox0);
