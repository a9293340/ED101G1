<?php 
    ...
    if(isset($_FILES["opImage"])){
        try {
            ...
            if($_FILES["opImage"]["error"] != 4){
                $sql1 = "update `OTHER_PRODUCT` set opName=:opName, opPrice=:opPrice, opClass=:opClass, opImage='',opStatus=:opStatus where opId=:opId";
                ...
                $products->bindValue(":opId", $_POST["opId"]);
                $products->execute();
            }else{
                $sql1 = "update `OTHER_PRODUCT` set opName=:opName, opPrice=:opPrice,opClass=:opClass, opStatus=:opStatus where opId=:opId";
                $products = $pdo->prepare($sql1);
                $products->bindValue(":opName", $_POST["opName"]);
                $products->bindValue(":opPrice", $_POST["opPrice"]);
                $products->bindValue(":opClass", $_POST["opClass"]);
            ...
            switch($_FILES["opImage"]["error"]){
                case 0:
                    ...
                    if(copy($from, $to)){
                        ...
                        $fixImage->bindValue(":opId",$_POST["opId"] );
